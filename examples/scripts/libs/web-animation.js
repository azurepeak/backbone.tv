 ...
 */
 ...
 /**
  * CUSTMIED FOR BACKBONE.TV
  * PR VERSION: https://github.com/web-animations/web-animations-js/pull/69
  * AUTHOR: @ragingwind
  */
  ...
  applyAnimatedValues: function() {
    for (var property in this.properties) {
      ...
      if (resultList.length > 0) {
        ...
        if (i == -1) {
          // @DIGGING: this method make reset the property.
...
DEFAULT_GROUP._tick = function(parentTime) {
  ...
  this.children.forEach(function(child) {
    ...
    // select finished animation or animation group.
    parentTime > child.endTime && finishedChilds.push( child );
  }.bind(this));
  ...
  // remove and reslove animations after done
  finishedChilds.forEach(function(child) {
    var target;
    if (child instanceof AnimationGroup) {
      target = child.children[0].targetElement;
    } else {
      target = child.targetElement;
    }

    if (target._animDeferred) {
      target._animDeferred.resolve(child);
      delete target._animDeferred;
    }
  }.bind(this));

  allFinished && ( this.children = [] );
