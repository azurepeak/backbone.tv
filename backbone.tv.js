...
(function(root, factory) {
  ...
  _.extend(XHRWebWorker.prototype, {
    request: function(resource, callback) {
      ...
      if (this.worker.requestQueue[resource.path] === undefined) {
        this.worker.requestQueue[resource.path] = new Array();
      }
  ...
  var Task = Backbone.TV.Task = function(options) {
    options = options || {};
  ...
  Task.prototype.run = function() {
    ...
    return options.run.call(options.context, options);
  }
      ...
      KeyboardEvent.bind(function(keyevent) {
        $tv.submit(new Task({
          ...
          run: function(options) {
            KeyboardEvent.trigger(options.keyevent);
          }
        }));
      });
  ...
  Resource.prototype.resolve = function() {
    ...
    if (this.deferred.retain == 0) {
      this.deferred.resolveWith(this.context || this.deferred);
    }
  }

  window.R = Backbone.TV.R = _.extend(Backbone.Events, {
    ...
    fetchTemplate: function(resource, callback) {
      if ($tv.xhrWebWorker) {
        $tv.xhrWebWorker.request(resource, function(resource, contents, status, xhr) {
    ...
    fetchImage: function(resource, callback) {
      ...
      var img = new Image();
      $(img).bind('load', function() { callback(resource, img, 'success'); })
    ...
    load: function(resources, context) {
      // create and add retain count to deferred as attribute for download counting.
      ...
      _.each(resources, function(resource) {
        var uri = _.extractUri(resource.uri), task;
        deferred.retain++;
        task = new Task({
          type: 'resource',
          resource: new Resource({
            type: uri[0],
            path: $tv.path(uri[0], uri[1]),
            context: context,
            deferred: deferred
          }),
          run: function() {
            var resource = this.options.resource;
            if (R.cache(resource.path) === undefined) {
              R.fetch(resource, function(resource, contents, status) {
                if (status === 'success') {
                  R.cache(resource.path, contents);
                }
                else {
                  console.error('Download resource is failed', resource.path);
                }
    ...
    get: function(resource) {
      var uri = _.extractUri(resource.uri);
  ...
  Backbone.TV.View = Backbone.View.extend({
    ...
    set: function(subview, sibling, append) {
      ...
      subview.render = function() {
        ...
        if (resourceDeferred) {
          $.when(resourceDeferred.promise()).then(viewRender);
        }
        else {
          viewRender(subview);
        }
  ...
  Backbone.TV.View.animate = function(options) {
    var group = undefined, animations = options.animations || [];

    // create animations with group. it's only supports ParGroup currently,
    // ignore timing arguments and convert delay time to startTime for
    // AnimationGroup
    if ( animations.length > 0 ) {
      var startTime = options.delay >= 0.5 ? options.delay : undefined;
    ...
    _.each( animations, function( anim ) {
      ...
      new Animation( anim.target, anim.effect, new Timing(anim), group );
    });

    // make keep only one deferred of animation per each element.
    if ( animations[0].target._animDeferred ) {
      ( delete animations[0].target._animDeferred );
    }
    return animations[0].target._animDeferred = $.Deferred();
  }
  ...
  Backbone.TV.Screen = Backbone.TV.View.extend({
    ...
    screen: function() {return $('div[data-backbonetv-type=screen]');},
